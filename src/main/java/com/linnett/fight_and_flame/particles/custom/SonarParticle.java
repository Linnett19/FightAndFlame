...
public class SonarParticle extends TextureSheetParticle {
    ...
    private float fadeG;
    private float fadeB;
    protected SonarParticle(ClientLevel world, double x, double y, double z, float xRot, float yRot) {
        super(world, x, y, z, 0.0, 0.5, 0.0);
        this.xd = 0.0;
        this.yd = 0.0;
        this.zd = 0.0;
    ...
    public void tick() {
        ...
        Vec3 motionVec = new Vec3(0, 0.2F, 0);
        this.xd += motionVec.x * f2 * 0.5F;
        this.yd += motionVec.y * f2 * 0.5F;
        this.zd += motionVec.z * f2 * 0.5F;
    ...
    private void renderSignal(VertexConsumer consumer, Camera camera, float partialTicks, Consumer<Quaternionf> rots) {
        ...
        Vector3f vector3f = (new Vector3f(1.0F, 0.0F, 0.0F)); // Ось X
        Quaternionf quaternionf = (new Quaternionf()).setAngleAxis((float) Math.toRadians(90), vector3f.x(), vector3f.y(), vector3f.z()); // Вращение на 90°
    ...
    public static class Provider implements ParticleProvider<SimpleParticleType> {
        ...
        public Particle createParticle(SimpleParticleType typeIn, ClientLevel worldIn, double x, double y, double z, double xSpeed, double ySpeed, double zSpeed) {
            SonarParticle particle = new SonarParticle(worldIn, x, y, z, (float) xSpeed, (float) ySpeed);
