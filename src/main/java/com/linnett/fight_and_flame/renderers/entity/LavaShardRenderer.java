package com.linnett.fight_and_flame.renderers.entity;
...
@OnlyIn(Dist.CLIENT)
public class LavaShardRenderer extends ThrownItemRenderer<LavaShardEntity> {
    private static final ResourceLocation TRAIL_TEXTURE = new ResourceLocation(FightAndFlame.MOD_ID, "textures/particle/trail.png");
    private static final float[][] COLORS = new float[][]{{0.08f,0.05f,0.01f},{0.04f,0.01f,0.08f},{0.01f,0.07f,0.06f},{0.01f,0.04f,0.06f}};
    public LavaShardRenderer(EntityRendererProvider.Context p_174416_) {
        super(p_174416_, 1, false);
    }

    @Override
    public void render(LavaShardEntity pEntity, float pEntityYaw, float pPartialTicks, PoseStack pMatrixStack, MultiBufferSource pBuffer, int pPackedLight) {
        if (pEntity.hasTrail()) {
    ...
    private void renderTrail(LavaShardEntity entityIn, float partialTicks, PoseStack poseStack, MultiBufferSource bufferIn, float trailR, float trailG, float trailB, float trailA, int packedLightIn) {
